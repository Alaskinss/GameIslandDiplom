            ...
            <DialogPanel
              ...
              <header class="py-3 px-4 flex justify-between items-center">
                <DialogTitle as="h3" class="text-lg leading-6 font-medium text-gray-900">
                  {{ user.id ? `Изменить профиль: "${props.user.name}"` : 'Создать нового пользователя' }}
                </DialogTitle>
                <button
                  @click="closeModal()"
              ...
              <form @submit.prevent="onSubmit">
                <div class="bg-white px-4 pt-5 pb-4">
                  <CustomInput class="mb-2" v-model="user.name" label="Логин"/>
                  <CustomInput class="mb-2" v-model="user.email" label="Почта"/>
                  <CustomInput type="password" class="mb-2" v-model="user.password" label="Пароль"/>
                </div>
